//
//  StatisticsSpec.swift
//  Gizmo_Tests
//
//  Created by Nayanda Haberty on 03/08/22.
    ...
    override func spec() {
        context("int") {
            ...
            it("should find an even median") {
                expect(arrayEven.median).to(equal(.double(3, 4)))
                expect(arrayEven.median.value).to(equal(3.5))
            }
            it("should find an odd median") {
                expect(arrayOdd.median).to(equal(.single(3)))
